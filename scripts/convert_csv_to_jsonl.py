...
def snake_case(s: str) -> str:
    s = s.strip().lower()
    # vervang euro/rare tekens door spatie
    s = re.sub(r"[€]", " ", s)
    # spaties/strepen/slashes -> underscore
    s = re.sub(r"[\s/]+", "_", s)
    s = re.sub(r"[-]+", "_", s)
    # verwijder overige niet-alfanum behalve underscore
...
def parse_value(v: str):
    ...
    # verwijder euro en duizendtallen-spaties
    v_clean = v.replace("€", "").replace("\u00a0", " ").strip()

    # getal detectie: 1.300 of 1 300 of 39,6
    v_digits = v_clean.replace(" ", "")
    # als het exact op een nummer lijkt
    if _num_re.match(v_digits):
        # 1.300 kan NL duizendtallen zijn -> maak er 1300 van als er 3 cijfers na punt staan
        ...
        try:
            f = float(v_digits)
            # int teruggeven als het een mooi geheel getal is
            if f.is_integer():
                return int(f)
            return f
        except ValueError:
...
def sniff_dialect(sample: str):
    # jouw bestand lijkt ; te gebruiken, maar we snuffen voor de zekerheid
    try:
        return csv.Sniffer().sniff(sample, delimiters=";,\t|")
    except Exception:
        # fallback: ; is waarschijnlijk
        class D:
            delimiter = ";"
        return D()
...
def convert(csv_path: Path, jsonl_path: Path, normalize_headers: bool = True):
    ...
    # Excel exports hebben soms een lege eerste regel, of rare BOM
    text = text.lstrip("\ufeff")
    ...
    with jsonl_path.open("w", encoding="utf-8") as f_out:
        for row in reader:
            ...
            # optioneel: skip volledig lege regels
